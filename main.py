...
def get_filtered_ai_signals(symbol: str, confidence_threshold: float = 70.0) -> List[Dict]:
    ...
    if not filtered_preds:
        filtered_preds = [{"symbol": symbol, "signal": "HOLD", "confidence": 0}]
...
def update_ai():
    ...
    return {
        "status": "success",
        "message": "Data refreshed and AI re-trained",
        "results_saved": True
    }
    ...
    try:
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "You are an expert AI trading assistant. Return ONLY JSON with trading signals."
                },
                {
                    "role": "user",
                    "content": "Generate signals for FOREX and CRYPTO pairs including symbol, signal, confidence, stop_loss, take_profit. Return only JSON array."
                }
            ],
            temperature=0.6,
            max_tokens=2000
...
def execute_trade(username: str, symbol: str, action: str):
    if username not in users_db:
        raise HTTPException(status_code=404, detail="User not found")
    return {"status": "success", "message": f"Executed {action.upper()} on {symbol} for {username}"}
...
def chat_with_ai_user(username: str, chat: ChatMessage):
    ...
    try:
        ...
        return {"success": True, "username": username, "response": ai_response, "timestamp": datetime.utcnow().isoformat()}
    except Exception as e:
        return {"success": False, "error": str(e)}
...
def get_trading_advice(symbol: str):
    ...
    try:
        response = client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": "You are a professional trading analyst. Return ONLY JSON advice."},
                {"role": "user", "content": f"Provide detailed trading advice for {symbol.upper()}. Include entry, stop loss, take profit, risk management."}
            ],
            temperature=0.6,
            max_tokens=1500
        )
...
def get_market_analysis():
    ...
    try:
        response = client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[
                {"role": "system", "content": "You are an expert market analyst. Return ONLY JSON."},
                {"role": "user", "content": "Provide comprehensive market analysis for Forex and Crypto, include top opportunities and risk factors."}
            ],
            temperature=0.7,
            max_tokens=1500
        )
        ...
        content = re.sub(r"```json|```", "", content).strip()
        data = json.loads(content)
        return {"success": True, "generated_at": datetime.utcnow().isoformat(), "analysis": data}
    except Exception as e:
        return {"success": False, "error": str(e)}
